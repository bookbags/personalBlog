    ...
    >
      ...
      <el-form-item label="选择分类">
        <el-select v-model="form.blogTypeName">
          <el-option v-for="item in blogType" :key="item.id" :value="item.name">{{item.name}}</el-option>
        </el-select>
      </el-form-item>
...
import {addBlogApi} from "@/api/blog.js";
...
export default {
  ...
  methods:{
    async submitHandel(){//格式必须完全参照接口
      ...
      let data = await addBlogApi(submitData);
      console.log(data, "返回");
